...
class ServerPackProvisioner(
     ...
     *
     * Read from the shipped template in SPC's `server_files`-directory ([ApiProperties.defaultVariablesTemplate]) so
     * the operator-facing text — the comments explaining every setting — can be corrected without rebuilding the API,
     * exactly like the start-script templates next to it. Falls back to the copy bundled in the jar when that file is
     * absent, which keeps the API usable before `ApiWrapper.setup()` has populated the home-directory.
     */
    val variables: String
        get() {
            val template = apiProperties.defaultVariablesTemplate
            if (template.isFile) {
                runCatching { return template.readText() }
                    .onFailure { log.error("Could not read ${template.absolutePath}; using the bundled template.", it) }
            }
            return bundledVariablesTemplate()
        }

    /**
     * The `variables.txt` template as shipped inside the jar. The last resort behind [variables]: without it a home
     * directory that has not been set up yet, or a template the user deleted mid-run, would silently produce a server
     * pack with no `variables.txt` at all.
     */
    private fun bundledVariablesTemplate(): String =
        javaClass.getResourceAsStream("/de/griefed/resources/server_files/variables.txt")
            ?.bufferedReader()?.use { it.readText() }
            ?: throw IllegalStateException("variables.txt is missing from the ServerPackCreator jar.")
