...
IF NOT EXIST variables.txt (
    CALL:CRASHSERVER "ERROR^^! variables.txt not present. Without it the server can not be installed, configured or started."
)
...
IF %MC_MINOR% LEQ 16 (
    IF NOT %JAVA_VERSION% EQU 8 (
        IF NOT %JAVA_VERSION% EQU 11 (
            CALL:CRASHSERVER "Minecraft 1.16 and older requires Java 8 or 11 - found Java %JAVA_VERSION%"
        )
    )
) ELSE IF %MC_MINOR% LEQ 20 (
    IF %MC_MINOR% EQU 20 (
        IF [!MC_PATCH!]==[] (
            IF %JAVA_VERSION% LSS 17  (
                CALL:CRASHSERVER "Minecraft 1.17 until 1.20.4 requires Java 17 - found Java %JAVA_VERSION%"
            )
        ) ELSE (
            IF !MC_PATCH! LEQ 4 (
                IF %JAVA_VERSION% LSS 17  (
            ...
            ) ELSE IF %JAVA_VERSION% LSS 21  (
                CALL:CRASHSERVER "Minecraft 1.20.5 and newer requires Java 21 - found Java %JAVA_VERSION%"
            )
        )
    ) ELSE (
        IF %JAVA_VERSION% LSS 17  (
...
IF ERRORLEVEL 0 (
	...
	IF EXIST !FORGEJARLOCATION! (
		...
		CALL:DELETEFILESILENTLY forge-installer.jar
		CALL:CRASHSERVER "Something went wrong during the server installation. Please try again in a couple of minutes and check your internet connection."
	)
)
EXIT /B 0
...
IF %IMPROVEDFABRICLAUNCHERAVAILABLE%==200 (
	...
	IF %FABRICAVAILABLE%==200 (
		...
		CALL:CRASHSERVER "Fabric is not available for Minecraft %MINECRAFT_VERSION%^, Fabric %MODLOADER_VERSION%."
	)
	...
	IF ERRORLEVEL 0 (
		...
		IF EXIST "fabric-server-launch.jar" (
			...
			CALL:DELETEFILESILENTLY fabric-installer.jar
			CALL:CRASHSERVER "fabric-server-launch.jar not found. Maybe the Fabric servers are having trouble. Please try again in a couple of minutes and check your internet connection."
		)
	) ELSE (
		SET LAUNCHERJARLOCATION=fabric-server-launcher.jar
		SET MINECRAFTSERVERJARLOCATION=server.jar
...
IF %COUNT% EQU 0 (
	CALL:CRASHSERVER "Quilt is not available for Minecraft %MINECRAFT_VERSION%^, Quilt %MODLOADER_VERSION%."
) ELSE (
	...
	IF EXIST quilt-installer.jar (
		...
		IF EXIST quilt-server-launch.jar (
			...
			CALL:DELETEFILESILENTLY quilt-installer.jar
			CALL:CRASHSERVER "quilt-server-launch.jar not found. Maybe the Quilt servers are having trouble. Please try again in a couple of minutes and check your internet connection."
		)
	)
)
...
IF %COUNT% EQU 0 (
	CALL:CRASHSERVER "LegacyFabric is not available for Minecraft %MINECRAFT_VERSION%^, LegacyFabric %MODLOADER_VERSION%."
) ELSE (
	...
	IF EXIST legacyfabric-installer.jar (
		...
		IF EXIST fabric-server-launch.jar (
			...
			CALL:DELETEFILESILENTLY legacyfabric-installer.jar
			CALL:CRASHSERVER "fabric-server-launch.jar not found. Maybe the LegacyFabric servers are having trouble. Please try again in a couple of minutes and check your internet connection."
		)
	)
)
SET LAUNCHERJARLOCATION=fabric-server-launch.jar
...
IF NOT EXIST eula.txt (
	...
	IF !ANSWER!==!EXPECT! (
		...
		CALL:CRASHSERVER "User did not agree to Mojang's EULA. Entered: %ANSWER%. You can not run a Minecraft server unless you agree to Mojang's EULA."
	)
)
EXIT /B 0
...
:MODLOADER-SWITCH
	GOTO MODLOADER-SWITCH-%MODLOADER% 2>NUL || (
        CALL:CRASHSERVER "Incorrect modloader specified: %MODLOADER%"
	)
