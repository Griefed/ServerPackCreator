...
class ComponentResizer(
    ...
    private var minimumSize = Dimension(10, 10)
        set(value) {
    ...
    init {
        ...
        cursors[6] = Cursor.S_RESIZE_CURSOR
        cursors[9] = Cursor.NE_RESIZE_CURSOR
        cursors[12] = Cursor.S_RESIZE_CURSOR
    ...
    override fun mouseMoved(e: MouseEvent) {
        ...
        direction = 0
        when {
            location.x < dragInsets!!.left -> {
                direction += WEST
            }
            location.x > source.width - dragInsets!!.right - 1 -> {
                direction += EAST
            }
            location.y < dragInsets!!.top -> {
                direction += NORTH
            }
    ...
    private fun changeBounds(source: Component, direction: Int, bounds: Rectangle?, pressed: Point?, current: Point) {
        ...
        when {
            SOUTH == direction and SOUTH -> {
                var drag = getDragDistance(current.y, pressed!!.y, snapSize.height)
                val boundingSize = getBoundingSize(source)
                val maximum = (boundingSize.height - y).coerceAtMost(maximumSize.height)
                drag = getDragBounded(drag, snapSize.height, height, minimumSize.height, maximum)
                height += drag
            }
        }
        val layout = source.parent.layout as MigLayout
        layout.setComponentConstraints(source, components[source]?.format(width, height))
