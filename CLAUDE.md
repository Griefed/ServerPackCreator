...
| web-frontend | 2 (from 0) | Phase 4a (Vitest infra) done |
...
`ConfigEditor` `GlobalScope.launch` anti-pattern (app); settings-store `$q` coupling with a
latently-broken error path, and `jsconfig.json` pointing at a missing `tsconfig.json` (frontend).

**Current phase — 4 (frontend). Next:** 4b — decouple the settings store from `$q` (testable error
path), then begin the TS migration (add `tsconfig.json`; convert stores/boot first, then
components), then add a Quasar-plugin test harness for component tests (Vue Test Utils + happy-dom).
