...
- **Next:** Phase 4b — decouple the settings store from `$q` (testable error path), then begin
  the TS migration (add tsconfig.json, convert stores/boot first, then components), and add a
