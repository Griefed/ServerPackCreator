...
| clientside     | 113           | Extracted from `-app`; `BootVerifier` split + `packPostProcessor` hook; selection (MC-support gate) + setup-abort classification pinned; `MetadataScanner` dispatches through `ModScanner.scannerFor`; the locked-file browser download treats an aborted navigation as the download starting, which is the only way CurseForge's `/download` ever succeeds. A crash is now re-checked **twice** before it may stand — on the newest loader build, and (when it contradicts a declared server support) on other versions of the mod, since one build's crash and a clientside mod were previously indistinguishable evidence (2026-08-23) |
