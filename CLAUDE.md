...
| app            | 118           | Phase 2 largely complete; clientside engine extracted out, CLI verbs stay; + `VersionCheckerTest`, `EventServiceTest` and `RunConfigurationServiceTest` (all three previously untested) and a pin on `MigrationManager.LAMBDA_SUFFIX`; + the `ConfigEditorViewModel` memo guards and `SuggestionProviderTest` (GUI typing-path performance, 2026-08-17 — pinned by *call counts* and by set identity, never wall-clock) |
