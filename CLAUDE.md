...
| `modscanning` gains `ModJarScanner`, `DescriptorScanner`, `JsonDescriptorScanner`, `FabricFamilyScanner`, `QuiltPackScanner` and `ModScanner.scannerFor` / `ModScanner.quiltPackScanner`; `JsonBasedScanner` becomes `@Deprecated`; the `internal` `Scanner<T, U>` is gone | Additive. A plugin can implement a scanner for the first time (`Scanner<T, U>` was `internal`, so nothing outside `-api` could ever reference it — removing it breaks nobody). `JsonBasedScanner` deliberately did **not** gain the new abstract `read`, so a subclass compiled against it still compiles; it now warns and points at `JsonDescriptorScanner`. Every concrete scanner keeps its class name, its public members and its `scan(Collection<File>): List<ScannedMod>` signature. |
