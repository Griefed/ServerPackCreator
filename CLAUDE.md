...
| app            | 149           | Phase 2 largely complete; clientside engine extracted out, CLI verbs stay; + `VersionCheckerTest`, `EventServiceTest` and `RunConfigurationServiceTest` (all three previously untested) and a pin on `MigrationManager.LAMBDA_SUFFIX`; + the `ConfigEditorViewModel` memo guards and `SuggestionProviderTest` (GUI typing-path performance, 2026-08-17 — pinned by *call counts* and by set identity, never wall-clock); + `AmountStatsServiceTest`, `ModPackDuplicateCheckTest` and `RunConfigurationListMigrationTest` (web query-shape and the DBRef-to-embedded migration, 2026-08-17); + `ModPackHashQueryTest`, `RunConfigurationCollectionNameTest`, `DeclaredIndexCreatorTest` and `DeclaredIndexStartupTest` (persistence declarations, 2026-08-18 — that the hash lookup is limited so duplicates cannot raise `IncorrectResultSizeDataAccessException`, that the migration's collection name matches the mapping, that the declared indexes are created on `ApplicationReadyEvent` and that doing so is **never** a startup condition; all assert against Spring Data's own machinery — `PartTree`, `MongoMappingContext`, `MongoPersistentEntityIndexResolver` — so none needs a database); + two `WebServiceContextTest` guards that the five components this work added are *registered* and that both deferred jobs listen for `ApplicationReadyEvent` — every other test for them constructs the class directly and passes whether Spring created it or not |
