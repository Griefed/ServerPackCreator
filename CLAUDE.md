...
| app            | 140           | Phase 2 largely complete; clientside engine extracted out, CLI verbs stay; + `VersionCheckerTest`, `EventServiceTest` and `RunConfigurationServiceTest` (all three previously untested) and a pin on `MigrationManager.LAMBDA_SUFFIX`; + the `ConfigEditorViewModel` memo guards and `SuggestionProviderTest` (GUI typing-path performance, 2026-08-17 — pinned by *call counts* and by set identity, never wall-clock); + `AmountStatsServiceTest`, `ModPackDuplicateCheckTest` and `RunConfigurationListMigrationTest` (web query-shape and the DBRef-to-embedded migration, 2026-08-17); + `ModPackIndexCreationTest`, `ModPackHashQueryTest` and `RunConfigurationCollectionNameTest` (persistence declarations, 2026-08-18 — that the declared index is actually *created*, that the hash lookup is limited so duplicates cannot raise `IncorrectResultSizeDataAccessException`, and that the migration's collection name matches the mapping; all three assert against Spring Data's own machinery — `MongoPersistentEntityIndexResolver`, `PartTree`, `MongoMappingContext` — so none needs a database) |
