...
| clientside     | 374           | Extracted from `-app`: platforms, metadata + server-boot signals, downloaders, fallback-list editor. **Four verdicts** (`CONFIRMED`/`CLEAR`/`ERROR`/`INCONCLUSIVE`) since 2026-09-04, and every clientside-determining rule lives in the bundled `boot-rules.default.json`. **The console decides; metadata only declares** — a `RuleSource.METADATA` rule may not carry a verdict. Full state, landmines and measurements: **`serverpackcreator-clientside/CLAUDE.md`**. |
