 ...
 * To use it, initialize it by calling {@link #initialize()}.
        ...
        try {
            ...
            if (jarResources != null) {
                ...
                try {
                    ...
                    LOG.debug(String.format("Language key \"%s\" not found in local file resource \"%s_%s\". Using fallback from JAR-resources.", languageKey, CURRENT_LANGUAGE.get(MAP_PATH_LANGUAGE), CURRENT_LANGUAGE.get(MAP_PATH_COUNTRY)));

                } catch (MissingResourceException ex2) {

                    value = FALLBACKRESOURCES.getString(languageKey);
                    LOG.debug(String.format("Language key \"%s\" not found in JAR-locale-resource \"%s_%s\". Using fallback from \"en_US\".", ex2.getKey(), CURRENT_LANGUAGE.get(MAP_PATH_LANGUAGE), CURRENT_LANGUAGE.get(MAP_PATH_COUNTRY)));
                ...
                value = FALLBACKRESOURCES.getString(languageKey);
                LOG.debug(String.format("Language key \"%s\" not found for locale \"%s_%s\". Using JAR-resources fallback from \"en_US\".", ex.getKey(), CURRENT_LANGUAGE.get(MAP_PATH_LANGUAGE), CURRENT_LANGUAGE.get(MAP_PATH_COUNTRY)));
